...
print("=" * 60)
print("Precomputing fair lending / geographic sensitivity sample")
print("=" * 60)
...
for chunk in pd.read_csv('data/df_model_raw.csv', low_memory=False, chunksize=50000):
    ...
    if len(sub) > 0:
        take = sub.sample(n=min(len(sub), 800), random_state=42)
        chunks.append(take)
        collected += len(take)
...
print(f"  Collected {len(df)} rows from 2016-2017 vintage across "
...
addr_state_col = df['addr_state'].copy()
...
df['state_default_rate'] = addr_state_col.map(
    config['state_default_map']).fillna(config['global_default_rate'])
df['state_lgd_rate'] = addr_state_col.map(
...
pd_X['addr_state'] = addr_state_col.values

print("\n[3/3] Scoring WITH state features vs. state NEUTRALIZED (set to global average)...")
X_with_state = pd_X[pd_features].copy()
X_without_state = pd_X[pd_features].copy()
X_without_state['state_default_rate'] = config['global_default_rate']

pd_with_state = pd_model.predict_proba(X_with_state)[:, 1]
pd_without_state = pd_model.predict_proba(X_without_state)[:, 1]

result_df = pd.DataFrame({
    'addr_state': pd_X['addr_state'].values,
    'pd_with_state': pd_with_state,
    'pd_without_state': pd_without_state,
})
result_df['pd_diff'] = result_df['pd_with_state'] - result_df['pd_without_state']

by_state = result_df.groupby('addr_state').agg(
    n=('pd_with_state', 'size'),
    avg_pd_with_state=('pd_with_state', 'mean'),
    avg_pd_without_state=('pd_without_state', 'mean'),
    avg_diff=('pd_diff', 'mean'),
).reset_index().sort_values('avg_diff', key=abs, ascending=False)

print(by_state.head(10).to_string(index=False))

joblib.dump({'by_state': by_state, 'overall_mean_diff': float(result_df['pd_diff'].mean()),
             'overall_mean_abs_diff': float(result_df['pd_diff'].abs().mean()),
             'sample_n': len(result_df)},
            'models/fairlending_state_sensitivity.joblib')
print("\nSaved: models/fairlending_state_sensitivity.joblib")
print("Done!")
